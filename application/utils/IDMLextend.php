...
class IDMLextend
{
  ...
  public function getSections($resource)
  {
    ...
    foreach ($resource as $content) {
		...
		for($i = 0; $i < $length - 1; ++$i) {
			...
			if ((strpos($loop_element[$i]['AppliedParagraphStyle'], 'ParagraphStyle/Section Divider') !== false)) {
				...
				array_push($section_index, explode("/",explode(" ", $loop_element[$i + 1]['AppliedParagraphStyle'])[0])[1]);

				var_dump(explode("/",explode(" ", $loop_element[$i + 1]['AppliedParagraphStyle'])[0])[1]);
				//var_dump(next($loop_element));
